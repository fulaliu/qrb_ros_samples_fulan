...
SendToLLM::SendToLLM(const std::string &name, const BT::NodeConfig &config,
                     rclcpp::Node::SharedPtr node)
    : BT::StatefulActionNode(name, config), node_(node) {
  pub_ = node_->create_publisher<std_msgs::msg::String>("/llm_input", 10);

  sub_ = node_->create_subscription<std_msgs::msg::String>(
      "/llm_output", 10, [this](const std_msgs::msg::String::SharedPtr msg) {
        if (!msg->data.empty()) {
          llm_reply_ = msg->data;
...
BT::NodeStatus SendToLLM::onRunning() {
  ...
  if (reply_received_) {
    ...
    RCLCPP_INFO(node_->get_logger(), "[SendToLLM] LLM reply received: \"%s\"",
                llm_reply_.c_str());
